...
- Treat private keys, seeds, addresses, identity records, balances, transactions, and QR/deeplink payloads as sensitive.
...
- For Verus Mobile runtime or simulator QA that requires a wallet, use only the testnet wallet named `antafritest`. Confirm both the Testnet profile selection and VRSCTEST network before any authenticated or state-changing action. Prefer its configured Face ID unlock; never request, retrieve, store, paste, log, or expose its password, seed, or private keys. Never unlock, inspect, or transact with a mainnet wallet for testing. If `antafritest`, Face ID, or the Testnet/VRSCTEST state is unavailable or ambiguous, stop and report runtime verification as blocked.
